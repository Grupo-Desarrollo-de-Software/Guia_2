...
public class CalculadorImpuestos {
    ...
    public double calcularPago(boolean descProntoPago, boolean descServicioPublico, boolean descTrasladoCuenta) {
        ...
        double impuestoNeto = darVehiculoActual().darPrecio() * porcentaje / 100;
        // TODO: Encontrar el valor del pago de impuesto de acuerdo a los datos de entrada
        double descuentosTotales;
        double descProntoPagoV = 0;
        double descServicioPublicoV = 0;
        double descTrasladoCuentaV = 0;
        if (descProntoPago){
            impuestoNeto = impuestoNeto - impuestoNeto * 0.1;
        }
        if (descServicioPublico){
            impuestoNeto = impuestoNeto - 50000;
        }
        if (descTrasladoCuenta){
            impuestoNeto = impuestoNeto - impuestoNeto * 0.05;
        }
        pago = impuestoNeto;
        return pago;
    }
    ...
    public Vehiculo buscarVehiculoPorMarca(String marca) {
        ...
        for (Vehiculo vehiculo : vehiculos) {
            if (vehiculo.darMarca().equals(marca)) {
                buscado = vehiculo;
                break;
            }
        }
    ...
    public Vehiculo buscarVehiculoMasAntiguo() {
        ...
        for (Vehiculo vehiculo : vehiculos) {
            if (Integer.valueOf(vehiculo.darAnio()) < modelo) {
                modelo = Integer.valueOf(vehiculo.darAnio());
                buscado = vehiculo;
            }
        }
        return buscado;
    ...
    public double promedioPreciosVehiculos() {
        ...
        for (Vehiculo vehiculo:vehiculos
            ...
            promedio = promedio + vehiculo.darPrecio();
        }
